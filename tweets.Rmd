...
a <- MMM_2020 %>% group_by(screen_name) %>% add_tally() %>%   filter(n() > 5) %>% ungroup()

a %>% mutate(screen_name = fct_reorder(screen_name,n)) %>% ggplot(aes(x= screen_name)) + geom_bar() + coord_flip()
